...
const NavBar = () => {
  ...
  return (
            ...
            {["home", "services", "about", "work", "contact"].map((section,index)=>(
                <div key={index} ref={(el)=>(linkRef.current[index]=el)}>
                    <a 
                        ...
                        offset={0} 
                        duration={2000} 
                        className='transition-all duration-300 hover:text-white cursor-pointer'>{section}</a>
                </div>
                )
            )}
        </div>
    ...
    <div style={showBurger?{clipPath: "circle(50.1% at 50% 50%)"}:{clipPath: "circle(0% at 50% 50%)"}} onClick={toggleMenu} className="fixed z-50 flex flex-col items-center justify-center gap-1 transition-all duration-300 bg-black rounded-full cursor-pointer w-14 h-14 md:w-20 md:h-20 top-4 right-10">
